    ...
    img.onload = function() {
      ...
      if(cfg.apiPath){
        ...
        fetch(cfg.apiPath+"save", {
            ...
            let newItem = {filename: data.filename, title: ''}
        ...
        let newItem = {filename: base64Image, title: ''}
        item[key].push(newItem);
        console.log(item);
        item = item;
        uploading = false;
...
<div class="row">
  <div class="col-md-1"><div class="box" style="background-image: url({cfg.imgPath}{img.filename});"></div></div>
  <div class="col-md-5">{#if cfg.imgTitle}<input type="text" class="form-control" bind:value="{item[key][i].title}" placeholder="{cfg.imgTitle}" />{/if}</div>
  <div class="col-md-6">
